...
function main() {
  ...
  // Allocate: top 11 → defense, rest → offense
  const defenders = teams.slice(0, 11);
  const offenders = teams.slice(11);

  // Sort defenders by defScore ascending (best defense first) for zone placement
  defenders.sort((a, b) => (a.defRaw ?? 9999) - (b.defRaw ?? 9999));

  const BR = defenders.slice(0, 4);   // bottom right — wall, gates BL
  const TOP = defenders.slice(4, 8);  // top — most points
  const BL = defenders.slice(8, 11);  // bottom left — gated, weakest
  ...
  printZone("TOP (4 slots)", 1000, TOP);
  printZone("BOTTOM RIGHT (4 slots, gates BL)", 550, BR);
  printZone("BOTTOM LEFT (3 slots, gated)", 550, BL);
